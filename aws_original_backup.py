...
if not st.session_state.connected:
    ...
    with tab1:
                        ...
                        if cols.empty:
                            ...
                            for _, row in tables.iterrows():
                                ...
                                is_enum_table = any(pattern in table_lower for pattern in [
                                    ...
                                    'complete_by', 'job_truck_unit', 'dispath_ordrer',  'attribiute', 'transcription_field', 'entity_note'
                                ])
                            ...
                            # Store exclusion details in session state with schema key
        ...
        # Display persistent exclusion list if available
        if sel_schemas:
            exclusion_key = f"excluded_tables_{'_'.join(sorted(sel_schemas))}"
        ...
        if st.session_state.erd_generated and st.session_state.erd_data:
            ...
            with col3:
                # Calculate and display total size from cached metadata
                total_size_gb = 0
                for schema in sel_schemas:
                    cache_key = f"{st.session_state.connection_params.get('environment', 'QA')}_{schema}"
                    if cache_key in st.session_state.schema_metadata:
    ...
    with tab3:
        ...
        # Define env2 outside the column context
        env2 = compare_env if st.session_state.connected else 'UAT'
        
        # Show schema differences after both environments are defined
        if st.session_state.connected and env2 in st.session_state.env_schemas:
            with col1:
                ...
                if only_in_env1:
                    st.error(f"🔴 Only in {env1}: {', '.join(sorted(only_in_env1))}")
        ...
        if env1 and env1 in st.session_state.env_connections and env2 in st.session_state.env_connections:
            ...
            with col2:
                ...
                # Show schema count and differences
                st.info(f"💾 {len(schemas2)} schemas available")
                
                # Show schemas only in this environment
                if env1 in st.session_state.env_schemas:
                    schemas1_set = set(st.session_state.env_schemas[env1])
                    schemas2_set = set(schemas2)
            ...
            if st.button("🔍 Compare Schemas"):
                if schema1 and schema2:
                    ...
                    # Table comparison with highlighting
                    ...
                    # Column comparison for common tables with highlighting
                    if common:
